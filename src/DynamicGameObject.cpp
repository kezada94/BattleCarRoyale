...
DynamicGameObject::DynamicGameObject(const char* path, const char* texture_path, GLuint shaderprog, btScalar mass, btVector3 startPos, btQuaternion startRot, btCollisionShape* coll)
    : GameObject(path, texture_path, shaderprog, mass, startPos, startRot, coll) {
        //this->getRigidBody()->setActivationState(DISABLE_DEACTIVATION);      
    }

DynamicGameObject::DynamicGameObject(const char* path, const char* texture_path, GLuint shaderprog, btScalar mass, btVector3 startPos, btQuaternion startRot)
    : GameObject(path, texture_path, shaderprog, mass, startPos, startRot) {
        //this->getRigidBody()->setActivationState(DISABLE_DEACTIVATION);      
    }
